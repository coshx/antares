        ...
        if (line[0].isdigit()) and ("->" not in line):
            ...
            with DRIVER.session() as session:
                ...
                session.write_transaction(
                    create_relationships, node_ids[0], node_ids[1], tree_id, new_relationship)
...
def create_relationships(
    ...
    AND a.tree_id = {tree_id} AND b.tree_id {tree_id}
...
def check_if_relationship_exists(tx, parent_node_identifier, tree_id, relationship):
    query = ''' 
    MATCH (a) WHERE a.identifier = {parent_node_identifier} AND (a)-[:{relationship}]->()
    RETURN a.tree_id = {tree_id}
